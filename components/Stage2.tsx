...
const Stage2: React.FC<Stage2Props> = ({ state, onVariableChange, onExecute, onComplete, isLoading }) => {
    ...
    const highlightTemplate = (template: string) => {
        ...
        return parts.map((part, index) => 
            /\{[^}]+\}/.test(part)
                ? <span key={index} className="bg-cyan-500/20 text-cyan-400 font-mono rounded-md px-1">{part}</span>
                : part
        );
    };
        ...
        >
            <div className="grid md:grid-cols-2 gap-8">
                ...
                <div className="space-y-6 p-6 bg-gray-800/50 border border-cyan-700/30 rounded-lg shadow-2xl shadow-cyan-900/20 flex flex-col">
                    <div>
                        <label className="block text-sm font-medium text-cyan-400">Template do Prompt</label>
                    ...
                    <div>
                        <label htmlFor="variableInput" className="block text-sm font-medium text-cyan-400">{variableLabel}</label>
                        <input 
                            id="variableInput" 
                            type="text" 
                            value={variableValue} 
                ...
                <div className="flex flex-col h-[400px] bg-gray-800/50 border border-cyan-700/30 rounded-lg shadow-2xl shadow-cyan-900/20">
                    <div className="p-4 border-b border-cyan-700/30">
                        <h3 className="text-lg font-semibold text-cyan-400">Prompt Gerado</h3>
                        <p className="text-gray-400 text-sm mt-2 h-20 overflow-y-auto custom-scrollbar">{generatedPrompt}</p>
                        <button onClick={onExecute} disabled={!generatedPrompt || isLoading} className="mt-3 w-full bg-blue-600 text-white font-bold py-2 px-4 rounded-md hover:bg-blue-500 disabled:bg-gray-600 disabled:cursor-not-allowed transition-all duration-300 flex items-center justify-center">
